...
Backend (FastAPI + Python)
  │
  ├── Typhoon Audio (typhoon-audio-preview)  → ถอดเสียงเป็นข้อความ
  ├── Typhoon v2 (typhoon-v2-70b-instruct)   → แก้ transcript + วิเคราะห์ + สรุป
...
### 1. สมัคร Typhoon API Key

ไปที่ https://opentyphoon.ai แล้วสมัครรับ API key
...
pip install fastapi uvicorn python-multipart openai httpx

# ตั้งค่า API Key (เลือก 1 วิธี)

# Windows PowerShell:
$env:TYPHOON_API_KEY="sk-xxxxxxxxxxxxxxxxxxxxxxxx"

# macOS / Linux:
export TYPHOON_API_KEY=sk-xxxxxxxxxxxxxxxxxxxxxxxx
...
User อัปโหลดไฟล์เสียง (.wav/.mp3 ฯลฯ)
  ...
  POST /api/v1/audio/upload
        ...
        └───────────────┬───────────────────┘
                        ...
                        ▼
        ┌─────────────────────────────────┐
        │  Typhoon Audio API              │  Speech-to-Text
        │  (typhoon-audio-preview)        │  ส่งทีละ chunk / ทั้งไฟล์
        └───────┬─────────────────────────┘
                │
                ▼
        ┌───────────────────────────────┐
        ...
        │
        ├── ★ ยิง n8n Webhook (ถ้าตั้งค่า N8N_WEBHOOK_URL)
        │
        ▼
...
### AI Pipeline (Typhoon Audio + Typhoon v2)
- **Typhoon Audio** — ถอดเสียงภาษาไทย/อังกฤษ, temperature 0.0 (deterministic), เหมาะกับภาษาไทยโดยเฉพาะ
- **Typhoon v2 70B** — แก้คำผิด + แก้ชื่อแบรนด์/สินค้าเป็นอังกฤษ + วิเคราะห์รวมใน call เดียว:
...
- ลบคำแปลก / STT hallucination ~70 คำ (ฝรั่งเศส, สเปน, เยอรมัน ฯลฯ)
- เพิ่มคำ hallucination ได้ที่ `_STT_HALLUCINATIONS` ใน `typhoon_ai_service.py`

### Queue System
- อัปโหลดหลายไฟล์พร้อมกัน → เข้าคิวอัตโนมัติ
- ประมวลผลทีละ 1 ไฟล์ → ป้องกัน rate limit
- พัก 15 วินาทีระหว่างไฟล์
...
### n8n Integration (ไม่ต้องแก้ไข)
- ยิง webhook ไปหา n8n หลังวิเคราะห์เสร็จอัตโนมัติ
- Payload โครงสร้างเดิม ไม่เปลี่ยนแปลง — n8n ทำงานได้ทันทีโดยไม่ต้องแก้ workflow
...
├── services/
│   ├── typhoon_ai_service.py        # ★ Typhoon AI Pipeline
│   │                                #   Typhoon Audio STT+Chunking, Language Filter, Typhoon v2 Fix+Analyze
│   │                                #   Multi Key Rotation, Retry, Multi-brand support
│   ├── groq_ai_service.py           # (เก็บไว้เป็น reference — ไม่ได้ใช้งานแล้ว)
...
|----------|----------|-------------|
| `TYPHOON_API_KEY` | ✅ | Typhoon API Key หลัก |
| `TYPHOON_API_KEY_2` ... `_20` | Optional | Key เพิ่มเติม (สลับ round-robin) |
| `TYPHOON_API_KEYS` | Optional | หลาย key คั่นด้วย comma |
| `N8N_WEBHOOK_URL` | Optional | URL Webhook ของ n8n (ไม่ต้องแก้ n8n workflow) |
...
| Model | Task | หมายเหตุ |
|-------|------|----------|
| `typhoon-audio-preview` | Speech-to-Text | Typhoon Audio — เหมาะกับภาษาไทย, temperature 0.0 |
| `typhoon-v2-70b-instruct` | NLP Analysis | แก้ transcript + วิเคราะห์รวมใน call เดียว |

---

## Config ที่ปรับได้ (`typhoon_ai_service.py`)

| Parameter | Default | Description |
|-----------|---------|-------------|
| `TYPHOON_STT_MODEL` | `typhoon-audio-preview` | Typhoon Audio model |
| `TYPHOON_LLM_MODEL` | `typhoon-v2-70b-instruct` | Typhoon v2 LLM model |
| `TYPHOON_MAX_FILE_SIZE_MB` | 24 | chunk ถ้าเกินขนาดนี้ |
| `CHUNK_DURATION_SECONDS` | 300 | ตัดทุกกี่วินาที (300 = 5 นาที) |
| `DELAY_BETWEEN_STEPS` | 1 | พักระหว่าง STT → LLM (วินาที) |
| `_STT_HALLUCINATIONS` | ~70 คำ | คำที่ STT มักถอดผิด (เพิ่มได้) |
...
| AI (Cloud) | Typhoon API (typhoon-audio-preview STT + typhoon-v2-70b-instruct LLM) |
| Automation | n8n Webhook (ไม่ต้องแก้ workflow เมื่อเปลี่ยน AI model) |
...
2. **Typhoon API มี rate limit** — ใส่หลาย key เพื่อสลับอัตโนมัติ
...
8. **Upload limit** — 60MB
9. **n8n Webhook** — Payload โครงสร้างเดิม ไม่เปลี่ยน แม้จะเปลี่ยน AI backend แล้ว
10. **test_local_ai.py** — เตรียมไว้สำหรับเปลี่ยนเป็น local AI (faster-whisper + Ollama) ในอนาคต
...
### การเปลี่ยน AI Model
ระบบออกแบบให้เปลี่ยน AI provider ได้ง่าย — แก้แค่ 1 ไฟล์:
- **Service file**: `services/typhoon_ai_service.py` — เปลี่ยน model/API ที่นี่
- **Router import**: `routers/ai_task.py` บรรทัดที่ 30 — ชี้ไปที่ service ใหม่
- **n8n, frontend, routers อื่นๆ** ไม่ต้องแก้